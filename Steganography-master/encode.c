...
Status open_files(EncodeInfo *encInfo)
{
		...
		encInfo->fptr_src_image = fopen((const char*)encInfo->src_image_fname, "rb");
		...
		encInfo->fptr_secret = fopen((const char*)encInfo->secret_fname, "rb");
		...
		encInfo->fptr_stego_image = fopen((const char*)encInfo->stego_image_fname, "wb");
...
Status do_encoding(EncodeInfo *encInfo)
{
		...
		encode_int_size_expression(encInfo->secret_extn_len, encInfo);
		...
		encode_magic_string((const char*)(encInfo->extn_secret_file), encInfo);
		...
		encode_int_size_expression(encInfo->size_secret_file - CHAR_SIZE, encInfo);
		...
		copy_remaining_image_data((FILE*) encInfo->fptr_src_image,(FILE*) encInfo->fptr_stego_image, encInfo->image_capacity - encInfo->magic_string_size + CHAR_SIZE);
